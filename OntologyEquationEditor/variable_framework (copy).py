...
def simulateDeletion(variables, var_ID):
  """
...
class Variables(OrderedDict):
  ...
  def indexVariables(self):
    ...
    self.indexEquationsInNetworks()

    for ID in self:
      for eq_ID in self[ID].equations:
        nw = self[ID].equations[eq_ID]["network"]
        self.index_equation_in_definition_network[nw].append((ID, eq_ID))
    ...
    if self.language in LANGUAGES["documentation"]:
      ...
      for ID in self.index_structures:
        try:
          ...
          print("debugging -- there is no self.indices")
...
class Operator(PhysicalVariable):
  ...
  def Khatri_Rao_indexing(self, a, b):
    ...
    #   if len(a.index_structures) == 1:
    #     a_index_right_ID = None
    #   else:
    #     a_index_right_ID = copy.copy(a.index_structures[1])
    #   if len(b.index_structures) == 1:
    #     b_block_index_right_ID = None
    #   else:
    #     b_block_index_right_ID = copy.copy(b.index_structures[1])
    #
    #   if a_index_left_ID in base_indices:
    #     if b_block_index_right_ID:
    #       if a_index_left_ID in indices[b_block_index_left_ID]["indices"]:
    #         # case 1 | 2
    #         if a_index_right_ID in indices[b_block_index_right_ID]["indices"]:
    #           pattern = 1
    #           l_a_bound = 2
    #           l_b_bound = 3
    #         else:
    ...
    # if b_block_index_left_ID in b_block:
    #   if b_block_index_right_ID in b_block:
    #     pass
    #   else:
    #     pattern = 0
    #     VarError("not a valid Khatri-Rao product. index set is not compatible")

    # print("debugging -- pattern : ", pattern)
...
class ExpandProduct(BinaryOperator):
  def __init__(self, op, a, b, space):
    ...
    self.index_structures = sorted(list(_s))
    pass
...
class MaxMin(BinaryOperator):
  def __init__(self, op, a, b, space):
    ...
    # rule for index structures
    _debug = a.index_structures == b.index_structures
    if a.index_structures == b.index_structures:  # strictly the same
      self.index_structures = sorted(a.index_structures)
      ...
      raise IndexStructureError("add incompatible index structures %s"
                                % CODE[self.space.language][op] % (
                                      a.pretty_a_indices, b.pretty_b_indices))
  def __str__(self):
    language = self.space.language
...
class Instantiate(Operator):
  ...
  def __str__(self):
    ...
    s = CODE[self.language]['Instantiate'] % (self.arg)
    return s
...
class TotDifferential(Operator):
  def __init__(self, x, y, space):
    ...
    self.index_structures = sorted(indexSet)
...
class ParDifferential(Operator):
  def __init__(self, x, y, space):
    ...
    if x.index_structures == y.index_structures:
      self.index_structures = sorted(x.index_structures)
      print("diff reduce 0")
      set_index = True
      return

    try:
      self.index_structures = sorted(self.Khatri_Rao_indexing(x, y))
      ...
      self.index_structures = sorted(self.Khatri_Rao_indexing(y, x))
    ...
    #   return
    #
    # if (len(x.index_structures) == 1) and (len(y.index_structures) == 1):
    #   if x.index_structures[0] in self.space.base_indices:
    #     if y.index_structures[0] in self.space.block_indices:
    #       if x.index_structures[0] in self.space.indices[y.index_structures[0]]:
    #         print("diff reduce 1")
    #         set_index = True
    #         self.index_structures = y.index_structures
    #   if x.index_structures[0] in self.space.block_indices:
    #     if y.index_structures[0] in self.space.base_indices:
    #       if x.index_structures[0] in self.space.indices[y.index_structures[0]]:
    #         print("diff reduce 1")
    #         self.index_structures = x.index_structures
    #         set_index = True
    ...
    #
    # # if
    # #   if ind_ID in self.space.indices.block_indices:
    # #     if x`
    #
    # for structure in filter(lambda sett: '&' in sett, indexSet):
    #   structures = structure.split(' & ')
...
class Expression(VerboseParser):
  ...
  token Instantiate : '\b(instantiate)\b';
  ...
  ;
  Expression/e -> 'Instantiate' '\(' Identifier/i '\)'                    $e=Instantiate(i, self.space)
      | List/e
      | Term/e( sum/op Term/t                                             $e=Add(op,e,t,self.space)
      )*
  ;
  List/el -> '\[' Identifier/i                                            $el=List(i,self.space)
            ( ',' Identifier/j                                            $el.addItem(j)
            )*
          '\]'
  ;
  Term/t -> Factor/t (
  ...
  ;
  ProdIndex/i -> (Variable/j '&' Variable/k)                               $i = j + ' & ' + k
  ;
  ...
  Factor/fu ->
      ...
      | 'Product'/s  '\(' Expression/a ',' Identifier/u '\)'                 $fu=Product(a, u, self.space)
      | 'root'/s  '\(' Expression/a ',' Identifier/u '\)'
      | MaxMin/fmm   '\(' Expression/amm ',' Expression/bmm '\)'           $fu=MaxMin(fmm, amm, bmm, self.space)
